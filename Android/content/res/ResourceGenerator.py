...
def filesin(resdir, resmap, respath):
    '''
    Entrega lisa de tuple en que se tiene nombre de archivo y primer directorio
    en que se encuentra
    '''
    unique = set()
    out = []
    items = sorted(filter(lambda x: x[1], resmap[resdir]))
    updateFlag = all(zip(*resmap[resdir])[1])
...
def processres(rgprefix, classname, resmap, respath, classmapping):
    ...
    for fileid, x in enumerate(lista):
        ...
        if fext != '.xml':
            ...
            continue
        if classname == 'values': continue
        filename = os.path.join(respath, x[1], x[0])
        print x[1], x[0]
        with open(filename, 'rb') as f:
            xmlstr = f.read()
            xmlns = re.findall(r'xmlns:([^=]+)="([^"]+)', xmlstr)
            xmlns = dict(xmlns)
...
def processvalues(rgprefix, classname, resmap, respath, values):
    ...
    valueids = {}
    for fileid, x in enumerate(lista):
        if x[0] in ('public.xml', 'symbols.xml'): continue
        hexval = idtemplate.format(rgprefix, restype, fileid)
        filename = os.path.join(respath, x[1], x[0])
...
def _create_resource_manager(packagename, resourcepath=None, resman=None):
    ...
    resourcepath, directories, files = os.walk(resourcepath).next()
    resmap = {}
    ...
    valueids = {}
    _values_ = []
    if classmapping.has_key('values'):
    ...
    for keyclass in sorted(classmapping):
        ...
        if keyclass != 'styleable': classpairs.sort()
...
if __name__ == '__main__':
    _create_resource_manager('Android')
