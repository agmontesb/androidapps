...
class ScrolledList(BasicViews.baseWidget):
    ...
    def setGUI(self, options):
        ...
        if self.isTree:
            dshow = 'tree ' + dshow
            dcolumns = '#all'
        tree = idewidgets.TreeList(uFrame, name=self.id, show=dshow, columns=columnsId, displaycolumns=dcolumns)
    ...
    def setSelectListener(self, tvsListener):
        self.tree.bind('<<TreeviewSelect>>', tvsListener)
